...
foreach ($name_par as $value)
{
  ...
  class MyDB extends SQLite3 {
   ...
   //remove table initially incase there are new products added to text file.
    $statement = $db->prepare('DROP TABLE IF EXISTS waltons_par');
    $result = $statement->execute();
    //recreate table with new values from text file
    $statement = $db->prepare('CREATE TABLE IF NOT EXISTS waltons_par ( ID INTEGER PRIMARY KEY, Name TEXT, Par INT, UNIQUE(Name))');
    $result = $statement->execute();

    $contents = file('name_stock_waltons.txt');
    foreach($contents as $line) {
        $line = trim($line);  
    $word = "INSERT OR IGNORE INTO waltons_par (Name) VALUES ('" .$line ."')"; //Insert product name
    //echo $word;
...
while ($loop_amount > -1)
{
    ...
    $qry = $db->prepare('UPDATE waltons_par SET PAR = :par_set WHERE ID= :count');
